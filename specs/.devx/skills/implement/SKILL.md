...
6. Read the TDD test specifications:
...
8. Implement one checklist item at a time:
   - Follow the spec exactly — do not add features not described
   - After each checklist item, verify it against requirements.md
   - Ensure all TDD tests pass before moving to the next checklist item

9. After all checklist items are complete:
